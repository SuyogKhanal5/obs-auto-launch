...
# reclaim_focus_after_play in the editor itself, which still exist for other reasons) never
...
def run_clip_editor_space_bar_listener(editor_hwnd, on_toggle, stop_event):
    """Runs for the clip editor's lifetime on its own dedicated daemon thread: SetWindowsHookExW
    (like RegisterHotKey elsewhere in this file) has thread affinity, so installing and later
    unhooking it must both happen from this same thread, and that thread needs a real message
    loop for the hook to ever actually fire (per Microsoft's own docs: a low-level hook callback
    is delivered by sending a message to the installing thread, same as RegisterHotKey's WM_HOTKEY).
...
def _run_clip_editor(master_root, config, recording_state, on_close, icon):
    ...
    def start_space_bar_listener():
        try:
            ...
            editor_hwnd = user32.GetAncestor(wintypes.HWND(root.winfo_id()), GA_ROOT)
        except Exception:
            logging.exception("Clip editor: could not resolve the editor window for the space bar hotkey.")
            return
        logging.info("Clip editor: resolved editor window %s (root.winfo_id()=%s) for the space bar hotkey.", editor_hwnd, root.winfo_id())
        threading.Thread(
            target=run_clip_editor_space_bar_listener,
            args=(editor_hwnd, on_space_bar_pressed, space_bar_stop_event),
            daemon=True,
        ).start()
