...
- Packaged as a standalone app per OS so it's easy to identify and kill in your process manager (not a generic `python`/`pythonw` process): `OBSAutoRecorder.exe` on Windows, an AppImage or plain tarball on Linux, an `.app` bundle on macOS (built separately for Apple Silicon and Intel — no universal binary, see [Known limitations](#known-limitations))
...
- **Windows 10/11**, **Linux** (any distro — X11 session recommended, see the Wayland note below), or **macOS 14.4 (Sonoma) or newer** (Apple Silicon or Intel — separate native builds for each, no universal binary; see [Known limitations](#known-limitations)). The floor is 14.4 specifically, not just any Sonoma release, since that's what's needed for per-app game-audio isolation to work at all.
...
2. Download the installer for your OS from the [Releases page](../../releases/latest) and run it: `OBSAutoRecorderInstaller.exe` (Windows), `OBSAutoRecorderInstaller-linux` (Linux — mark it executable first, `chmod +x OBSAutoRecorderInstaller-linux`), or `OBSAutoRecorderInstaller-macos-arm64` / `OBSAutoRecorderInstaller-macos-x86_64` (macOS — arm64 for Apple Silicon, x86_64 for Intel; `chmod +x` it first, same as Linux).
...
- **macOS**: download `OBSAutoRecorder-macos-arm64.dmg` (Apple Silicon) or `OBSAutoRecorder-macos-x86_64.dmg` (Intel), open it, and drag `OBSAutoRecorder.app` to Applications. Add a `config.json` inside the app bundle's `Contents/MacOS/` folder (see step 2) — or just use the installer instead, which handles this for you.
...
On macOS specifically, this produces a binary for whichever architecture you run it *on* (no cross-compiling to the other one) — there's no universal2 wheel for `numpy`, so build on an Apple Silicon Mac for an Apple Silicon build, or an Intel Mac for an Intel one; the release workflow does exactly this by running on both a `macos-latest` (Apple Silicon) and a `macos-15-intel` (Intel) runner.
...
- **Show Audio Mixer Levels** — toggle live mixer levels on the overlay; connects to OBS for this whenever it's running, independent of whether a game is currently being recorded. If OBS isn't running yet, turning this on launches it automatically (same as **Start OBS**) instead of just sitting on "No active audio sources" until something else happens to start OBS — levels start showing themselves as soon as it's up. Turning this on also automatically picks a monitor for **Overlay Monitor** too if it was still set to "Off" (your primary monitor, where available), so the levels actually become visible instead of updating on a hidden overlay — it never overrides a monitor you've already chosen, and doesn't turn the overlay back off when you disable levels again.
...
- **Edit Clips...** — opens the built-in clip editor: browse for a recording (or pick one from a Recent Recordings list), preview it, pick a start/end point, and trim it. See [Optional: clip editor](#optional-clip-editor)
- **Edit Settings...** — opens a GUI settings window covering every `config.json` option (watched games/windows, launchers, OBS connection, cleanup/guards, post-processing, notifications) plus a **Custom Keybinds** tab and a **Clip Editor** tab, organized into tabs, with **Save** and **Save and Restart** buttons. No manual JSON editing needed. See [Optional: settings editor](#optional-settings-editor)
...
**Windows and macOS (14.4+)** — see [Known limitations](#known-limitations) for why Linux doesn't have this yet.
...
Right-click the tray icon → **Edit Clips...** for a simple built-in editor: preview a recording with real playback, pick a start and end point, and trim it to its own file — no separate video editor needed for the common case of "just cut this session down to the good part."

- **Open a recording** — **Browse for a recording...**, or pick one from the **Recent Recordings** dropdown (populated from `obs.output_folder`, newest first, including per-game subfolders if [game subfolders](#optional-game-subfolders) is on). The editor refuses to open whichever file OBS is actively recording to right now — wait for it to finish first.
- **Preview** — Play/Pause, **Rewind 5s**, drag the seek bar, and the current/total time. This is real VLC playback, not just a scrub-through-thumbnails preview.
- **Set the range** — **Set Start** / **Set End** capture the current playhead position into an editable timestamp field (`HH:MM:SS.mmm`, or just type an exact value by hand for frame-level precision beyond what scrubbing gives you).
- **Trim it** — **Trim Clip** runs in the background (the editor stays responsive) and saves to `clip_editor.output_folder` (or next to the source if left blank), never overwriting an existing file — a name collision gets ` (2)`, ` (3)`, etc. appended, the same convention Windows Explorer itself uses. A toast notification fires on completion if `notifications.enabled` is `true`, and either way the result also lands in `autostart_script.log`.
...
- **Game audio isolation** ([Optional: isolate game audio](#optional-isolate-game-audio)) works on **Windows and macOS (14.4+)**, not yet on Linux. On Windows it's built on OBS's WASAPI-specific process-audio-capture source; on macOS it's built on OBS's `sck_audio_capture` source (Apple's Core Audio process taps, macOS 14.4+, which is why that's this app's macOS floor — targeting a specific app there uses its bundle identifier, resolved automatically from the detected process, not something you need to look up yourself). Both were confirmed empirically against a real, running OBS instance rather than assumed from documentation — see [CROSS_PLATFORM_PLAN.md](CROSS_PLATFORM_PLAN.md) §6.2. Linux has no equivalent yet: an investigation into the real per-OS OBS source-type schema found that OBS's own Linux build doesn't currently expose a distinct per-application audio-capture source at all under its own generic PulseAudio-compatible capture kinds, at least in the configuration tested (§6.1). `obs.multi_track_audio.app_captures`-based routing has the same Linux gap in the meantime. One related rough edge on macOS: the small audio-track sync-offset correction this feature applies was measured specifically for Windows's WASAPI timing behavior and hasn't been separately measured for macOS's very different capture pipeline — it defaults to the Windows-measured value (harmless if it's simply not quite right, since it's just a starting point), but is fully overridable via `obs.process_audio_capture_sync_offset_ms` or this app's own live calibration flow if you want it tuned for real on your own Mac.
...
- **macOS has no single universal binary** — `numpy` doesn't publish a universal2 (combined Intel + Apple Silicon) wheel at all, so Apple Silicon and Intel each get their own separately-built `.app`/installer instead of one download that works on both. Make sure you grab the right one for your Mac (see [Quick install](#quick-install-recommended)).
