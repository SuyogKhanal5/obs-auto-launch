...
On macOS specifically, this produces a binary for whichever architecture you run it *on* (no cross-compiling to the other one) — there's no universal2 wheel for `numpy`, so build on an Apple Silicon Mac for an Apple Silicon build, or an Intel Mac for an Intel one; the release workflow does exactly this by running on both a `macos-latest` (Apple Silicon) and a `macos-13` (Intel) runner.
