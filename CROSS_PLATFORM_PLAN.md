...
- Epic Games Launcher / GOG Galaxy / Xbox Game Pass auto-detection on Linux/macOS (no native clients exist — see §2.1). Manual process-name configuration (already how Battle.net/Xbox work today) remains the fallback.
...
3. **Global hotkeys:** X11 only for v1 (same conceptual model as Windows' `RegisterHotKey` — a real global hotkey that fires regardless of focus). Wayland users get no in-app global hotkey for v1; this is a documented, known gap, not a bug to chase down. The same X11-vs-Wayland split applies to the clip editor's space-bar-while-video-focused behavior and to window-title detection (§2.1's Minecraft-style rules), since all three rely on similar low-level input/window APIs that don't exist on Wayland by design.
...
### Assumptions filled in where the questions didn't fully cover it (flag if wrong)

- **Epic/GOG/Xbox/Battle.net launcher auto-detection**: NOT ported to Linux/macOS. Steam gets full native auto-detection (real client exists on all 3 OSes). Epic/GOG/Xbox have no native Linux/macOS client at all; Battle.net has none either but already works via manual `install_dirs` config today, which needs no porting. This interpretation was not explicitly re-confirmed after the audio-isolation question — correct me if you actually want Heroic-Games-Launcher-style detection built too, since that's a materially larger, separate piece of work (different app, different manifest format per launcher).
- **macOS minimum version**: OBS's ScreenCaptureKit-based per-app audio capture effectively requires macOS 13 (Ventura) or newer. Proposing macOS 13+ as the floor; flag if you need to support older macOS.
- **Linux ffmpeg/OBS auto-install**: Windows uses `winget`. macOS gets the equivalent via Homebrew (checking for `brew` first, same "best-effort, never blocks setup" philosophy as today). Linux does NOT get an auto-install attempt — package manager fragmentation (apt/dnf/pacman/zypper, plus Flatpak) makes a reliable auto-install materially riskier than Windows/macOS; Linux users instead get clear per-distro manual instructions on the same installer screen. Flag if you'd rather attempt `apt`-only auto-install with graceful fallback.
- **Installer elevation model**: Windows installs to `Program Files` (needs admin, `--uac-admin`). Proposing Linux/macOS default to a **per-user** install location (`~/.local/share/OBSAutoRecorder` on Linux, `~/Applications` on macOS) specifically to avoid needing `pkexec`/`sudo`/`osascript`-elevation at all — a real scope reduction, not just a style choice. Flag if a system-wide (`/opt`, `/Applications`) install is required instead.
...
| `get_epic_installed_games(...)`, `get_gog_installed_games(...)`, `get_xbox_install_dirs(...)` | Win only | Linux/Mac backends return `[]`/`None` immediately — see §2's assumption |
...
| `install_optional_dependency(package)` (ffmpeg auto-install) | Win(winget)/Mac(brew)/Linux(no-op, returns "manual" status) | |
...
- `autostart_script.py:243-249` (`get_xbox_install_dirs`) — Xbox/Game Pass has no Linux/macOS client at all; per §2 assumption, backend returns `[]` on non-Windows.
- `autostart_script.py:252-261` (`get_battlenet_install_dirs`) — no registry/manifest lookup, just user-configured dirs; per §2, same treatment as Xbox (already effectively OS-agnostic mechanism, just no non-Windows client to point it at by default).
- `autostart_script.py:264-297` (`get_gog_installed_games`) — pure `winreg` enumeration (`HKLM\SOFTWARE\WOW6432Node\GOG.com\Games`); per §2, not ported.
- `autostart_script.py:300-302` (`get_epic_manifest_dir`), `305-337` (`get_epic_installed_games`) — hardcoded `%PROGRAMDATA%\Epic\...` + `.item` JSON manifests; per §2, not ported.
...
3. Confirm `get_xbox_install_dirs`/`get_battlenet_install_dirs`/`get_epic_installed_games`/`get_gog_installed_games` Linux/macOS backends return empty results cleanly (not errors) per the §2 assumption — these are intentionally thin/no-op backends, not full ports.
...
**Steps:**
1. CI spike: install real OBS on `ubuntu-latest`/`macos-latest`, launch it headless, connect via `obsws-python`, call `GetInputKindList`, record the actual available per-app-audio-capture input kind names and their `GetInputDefaultSettings` schema.
2. Implement `configure_process_audio_capture` per OS using the confirmed schema from step 1, with the capability-probe-and-log-clearly fallback design from §6 (not a hardcoded assumption).
3. Wire into the same higher-level multi-track-audio-profile setup flow the Windows WASAPI path already uses.
...
5. Default Linux/macOS installs to a per-user location (§2's elevation-avoidance assumption) — no `pkexec`/`osascript`-elevation code needed unless that assumption gets corrected.
6. ffmpeg auto-install: macOS via Homebrew detection (mirroring today's `winget` best-effort philosophy); Linux shows manual per-distro instructions instead of attempting an auto-install, per §2's assumption.
