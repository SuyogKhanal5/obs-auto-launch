...
**Verification status**: unit tests pass and the app runs stably with this change (compile-checked, full suite green, live-relaunched without crashing). The actual click-to-open-popup interaction itself could not be verified by automation in this session -- both AppleScript/System Events and `cliclick` (CGEventPost-based) require Accessibility permission for the process driving them that wasn't available to grant in this session, and neither delivered a click the app's own log showed it receiving. The fix is a deterministic code-level change (no native `NSMenu` is constructed on macOS at all anymore, full stop -- not a probabilistic mitigation), but real-hardware confirmation that the popup actually renders and dismisses correctly on a real click is still an open item for whoever next has hands-on access.
