...
5. Implement `configure_process_audio_capture` per OS only once step 4 (Linux) / real-hardware testing (macOS) actually confirms a real, working mechanism, with the capability-probe-and-log-clearly fallback design from §6 (not a hardcoded assumption) — **not started**, blocked on the above.
6. Wire into the same higher-level multi-track-audio-profile setup flow the Windows WASAPI path already uses.
...
**Exit criteria:** the CI spike from step 1 produces a concrete, verified answer (not a guess) about what OBS actually calls this feature on Linux/macOS, documented back into this plan file before implementation proceeds. This is explicitly allowed to reveal that full parity isn't achievable on some OS/OBS-version combination — if so, document that clearly rather than shipping a broken feature. **Status: met, with a negative result.** Both spikes ran for real and produced genuine, concrete findings (§6.1) rather than a guess: Linux's stock OBS build exposes no per-application audio capture on any input kind it ships (confirmed with real audio playing against a real, running PipeWire session, not just inferred from a static schema); macOS is confirmed blocked by the CI environment itself, not merely slow. Phase 6 implementation (steps 4-6) stays paused on the follow-up in step 4 and on real hardware access (macOS), rather than shipping a guessed `configure_process_audio_capture` — exactly the "document clearly rather than ship a broken feature" outcome this section anticipates.
...
**Exit criteria:** all 3 installers produce a working install from a single CI run on a tagged release, verified by each installer's own smoke test (install → confirm the app launches → uninstall cleanly) running on its native CI runner. **Status: partially met.** All 3 OSes build successfully via a real, triggered CI run (this is real verification, not aspirational) — Windows and Linux artifacts, and a macOS arm64 `.app`/`.dmg`. The full "install → launches → uninstall cleanly" smoke test itself is **not implemented** (would need real interactive install-wizard automation per OS, e.g. via a GUI-automation tool, which this phase didn't attempt) — the CI verification that did happen is "does the build itself succeed," a real and valuable but narrower check than the exit criteria's original ask. Flagged as follow-up, consistent with how this plan treats other real-hardware/interactive-only gaps (Phase 3's macOS hotkey happy path, Phase 4's NSView bridge).
