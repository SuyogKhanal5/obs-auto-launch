...
**Status: Phases 0-8 complete and CI-verified; this document is kept as the technical record of *why* the architecture looks the way it does and *what was actually confirmed working* (vs. guessed), not archived — read it before touching any OS-specific code path.** Phase 6 (game audio isolation) is implemented on Windows and macOS, both confirmed empirically against a real, running OBS instance (§6.1, §6.2); Linux remains a confirmed negative finding, not an unimplemented feature — see §6.1 before assuming this feature could exist there at all. For user-facing setup/usage docs, see [README.md](README.md), which now also has OS-specific instructions throughout (Phase 9).
