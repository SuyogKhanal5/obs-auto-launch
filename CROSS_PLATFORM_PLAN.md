...
**Status: Phases 0-8 complete and CI-verified; this document is kept as the technical record of *why* the architecture looks the way it does and *what was actually confirmed working* (vs. guessed), not archived — read it before touching any OS-specific code path.** Phase 6 (game audio isolation) is implemented on Windows and macOS, both confirmed empirically against a real, running OBS instance (§6.1, §6.2); Linux remains a confirmed negative finding, not an unimplemented feature — see §6.1 before assuming this feature could exist there at all. **§5.12-§5.13 are worth reading before assuming the macOS port works at all**: the very first real end-to-end run on real Mac hardware surfaced three separate, previously-uncaught crashes in sequence, each only reachable once the previous one was fixed — a Tk/AppKit main-thread conflict with `pystray` crashing on startup every time; opening the tray menu itself crashing next (a native `NSMenu` closing anywhere in the process crashes Tk's own macOS backend); and opening the clip editor's video preview crashing after that (the original NSView-embedding approach's core assumption was simply wrong — confirmed via a real `SIGSEGV`, fixed with a completely different, real mechanism found in python-vlc's own example, not a guess). All three are now fixed; the first two are confirmed by real usage (the app's own log shows a real session using the tray menu successfully), the third by an exact isolated reproduction of the crash now running clean. None of this was, or could have been, caught by CI, since CI can't launch a real GUI on macOS at all — "CI passes" and "actually confirmed to run" remain different claims on this OS specifically. For user-facing setup/usage docs, see [README.md](README.md), which now also has OS-specific instructions throughout (Phase 9).
